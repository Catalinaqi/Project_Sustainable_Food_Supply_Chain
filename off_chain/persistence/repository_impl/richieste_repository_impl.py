...
class RichiesteRepositoryImpl:
    ...
    def inserisci_richiesta(
        ...
        query_builder.table("Richiesta").insert(
    ...
    def get_richieste_ricevute(
        self, id_azienda: int, check_trasporto: bool = False
    ) -> List[RichiestaModel]:
        """
        Restituisce tutte le richieste ricevute da un'azienda.
        Se check_trasporto è True, filtra richieste accettate dove l'azienda è trasportatore.
        """
        try:
            query_builder = QueryBuilder()
            query_builder.select(
                "r.Id_richiesta",
                "r.Id_richiedente",
                "rich.Nome AS Nome_richiedente",
                "r.Id_ricevente",
                "rice.Nome AS Nome_ricevente",
                "r.Id_trasportatore",
                "tras.Nome AS Nome_trasportatore",
                "r.Id_prodotto",
                "prod.Nome AS Nome_prodotto",
                "r.Quantita",
                "r.Stato_ricevente",
                "r.Stato_trasportatore",
                "r.Data",
            ...
            if not check_trasporto:
                query_builder.where("r.Id_ricevente", "=", id_azienda)
            else:
                query_builder.where("r.Stato_ricevente", "=", db_default_string.STATO_ACCETTATA)
                query_builder.where("r.Id_trasportatore", "=", id_azienda)
            ...
            for r in risultati_raw:
                ...
                """
                res_lotto = self.db.fetch_one(
                    query_lotto,
                    (id_prodotto, db_default_string.TIPO_OP_PRODUZIONE, db_default_string.TIPO_OP_TRASFORMAZIONE),
                )
    ...
    def update_richiesta(self, id_richiesta: int, nuovo_stato: str, azienda_role: str) -> None:
        ...
        """
        query_builder = QueryBuilder()
        query_builder.table("Richiesta").where("Id_richiesta", "=", id_richiesta)

        if azienda_role == db_default_string.TIPO_AZIENDA_TRASPORTATORE:
            query_builder.update(Stato_trasportatore=nuovo_stato)
        elif azienda_role in (
            db_default_string.TIPO_AZIENDA_AGRICOLA,
            db_default_string.TIPO_AZIENDA_TRASFORMATORE,
        ):
            query_builder.update(Stato_ricevente=nuovo_stato)
        else:
